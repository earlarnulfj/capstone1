...
// ---- Staff auth guard ----
// Support multi-instance logins: check for token in URL/localStorage
// Each tab gets its own token stored in localStorage (which is per-tab)
$loginToken = $_GET['login_token'] ?? null;

// Check authentication - JavaScript will provide token via cookie on subsequent requests
// For initial load, check URL parameter, otherwise check all available logins
$staffData = null;
if ($loginToken) {
    $staffData = getRoleSession('staff', $loginToken);
}

// Fallback: Check all login instances for staff role (for pages loaded without token param)
if (!$staffData && isset($_SESSION['_logins'])) {
    foreach ($_SESSION['_logins'] as $token => $login) {
        if ($login['role'] === 'staff') {
            $staffData = $login['data'];
            break; // Use first available staff login
        }
    }
}

// Final fallback: direct session check (backward compatibility)
if (!$staffData) {
    if (isset($_SESSION['staff']) && is_array($_SESSION['staff']) && !empty($_SESSION['staff']['user_id'])) {
        $staffData = $_SESSION['staff'];
    }
}

if (!$staffData || empty($staffData['user_id']) || ($staffData['role'] ?? null) !== 'staff') {
    header("Location: login.php");
    exit();
}

// Store staff data for use in the page
$staffUserId = $staffData['user_id'];
$staffUsername = $staffData['username'] ?? 'Staff';
...
if (
    ...
    try {
        foreach ($items as $item) {
            ...
            $sales->user_id      = $_SESSION['staff']['user_id'] ?? 0;
    ...
    if ($item_id > 0) {
        ...
        try {
            ...
            foreach ($allVariations as $orderVar) {
                ...
                // Unit type from admin_orders - use latest
                ...
                // Price from admin_orders (completed orders)
            ...
            error_log("POS: Could not load data for inventory ID {$item_id}: " . $e->getMessage());
        }
...
<body>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div id="receiptContent" class="p-3">
            ...
            <p><strong>Cashier:</strong> <?= htmlspecialchars($_SESSION['staff']['username'] ?? 'Staff') ?></p>
            <h6>Items</h6>
  ...
  // Multi-instance login support: Store token in sessionStorage for this tab
  // sessionStorage is per-tab, allowing multiple tabs to have different tokens
  (function() {
    const urlParams = new URLSearchParams(window.location.search);
    const loginToken = urlParams.get('login_token');
    
    if (loginToken) {
      // Store token in sessionStorage (per-tab storage - each tab has its own sessionStorage)
      try {
        sessionStorage.setItem('login_token_staff', loginToken);
        // Remove token from URL after storing
        urlParams.delete('login_token');
        const newUrl = window.location.pathname + (urlParams.toString() ? '?' + urlParams.toString() : '');
        window.history.replaceState({}, '', newUrl);
      } catch(e) {
        console.warn('Could not store login token:', e);
      }
    }
    
    // Get stored token for this tab (from sessionStorage)
    let storedToken = null;
    try {
      storedToken = sessionStorage.getItem('login_token_staff');
    } catch(e) {}
    
    // Update logout link to include token
    const logoutLink = document.getElementById('logoutLink');
    if (logoutLink && storedToken) {
