...
if (!$isLocalHost && (($_SERVER['HTTPS'] ?? 'off') !== 'on') && (($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '') !== 'https')) {
    ...
    if (!isset($_GET['_https_redirect'])) {
        $requestUri = $_SERVER['REQUEST_URI'] ?? '/supplier/login.php';
...
// Calculate root APP_BASE (since we're in /supplier/, APP_BASE would be '/supplier' or '/haha/supplier', but we need the root)
...
if (preg_match('#/(admin|staff|supplier)$#', $rootBase)) {
    ...
    // If script is in /haha/supplier/, remove /supplier to get /haha
...
if (!isset($_SESSION['rate_limit']['supplier_login'])) $_SESSION['rate_limit']['supplier_login'] = [];
// Purge old entries
$_SESSION['rate_limit']['supplier_login'] = array_filter(
    $_SESSION['rate_limit']['supplier_login'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($_SESSION['csrf_token'], $csrf)) {
        ...
        // Rate limit check
        if (count($_SESSION['rate_limit']['supplier_login']) >= 10) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                // Attempt login by email - only for supplier
                // Note: Each browser session is independent, so multiple suppliers can log in simultaneously
                ...
                if ($result['success'] ?? false) {
                    // Verify it's actually a supplier account
                    if ($result['role'] === 'supplier') {
                        ...
                        // Redirect to supplier dashboard (use relative path from supplier folder)
                        header('Location: dashboard.php');
                        exit();
                    } else {
                        $errors[] = 'This account is not authorized for Supplier access.';
                        $_SESSION['rate_limit']['supplier_login'][] = time();
                    }
                } else {
                    // Map errors
                    ...
                    if ($err === 'invalid_email') {
                        ...
                        $errors[] = 'No supplier account found for that email address.';
        ...
        <div class="card login-card">
          <div class="card-body p-4">
            <h3 class="mb-3 text-center">
              <i class="bi bi-truck me-2"></i>Supplier Login
            </h3>
            ...
            <?php if (isset($_GET['success'])): ?>
              <div class="alert alert-success">
                Registration successful! You can now login with your Supplier account.
              </div>
            <?php endif; ?>
            ...
            <form method="POST" action="" id="loginForm" novalidate>
              ...
              <div class="mb-3">
                ...
                <div class="form-text">Use your supplier email address.</div>
              </div>
            ...
            <div class="mt-4">
              ...
              <div class="d-grid">
                <a href="<?php echo $rootBase; ?>/auth/google/login.php?role=supplier" class="btn btn-outline-danger">
            ...
            <div class="mt-4">
              ...
              <div class="d-grid">
                <a href="<?php echo $rootBase; ?>/auth/google/register.php?role=supplier" class="btn btn-outline-primary">
                  <svg width="18" height="18" class="me-2" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
