...
// For sidebar active highlight (optional)
$activePage = 'settings';

// Get user ID from namespaced admin session
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update') {
    ...
    $newRole     = trim($_POST['role']     ?? $role     ?? 'management'); // keep management by default
    ...
    if ($invalidPhone) {
        ...
        if ($user->update()) {
            // Refresh the model & update the session snapshot (header/sidebar use these)
...
<head>
  ...
  <title>Settings - Inventory & Stock Control System</title>
      ...
      <form method="POST">
        ...
        <div class="mb-3">
          ...
          <select class="form-select" id="role" name="role" required>
            ...
            <option value="management" <?= (isset($role) && $role === 'management') ? 'selected' : '' ?>>Management</option>
          </select>
        </div>
