...
if (isset($_GET['save_form_data']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $_SESSION['temp_registration_data'] = [
        ...
        'role' => 'management', // Fixed for admin
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $confirm_password = $_POST['confirm_password'] ?? '';
    $email = $_POST['email'] ?? '';
    $role = 'management'; // Fixed for admin registration
    $phone = $_POST['phone'] ?? '';
    $address = trim($_POST['address'] ?? '');
    $city = trim($_POST['city'] ?? '');
    $province = trim($_POST['province'] ?? '');
...
<head>
    ...
    <title>Admin Registration - Inventory & Stock Control System</title>
                ...
                <div class="card shadow">
                    <div class="card-body p-5">
                        <h2 class="text-center mb-4">
                            <i class="bi bi-shield-lock text-primary me-2"></i>Admin Registration
                        </h2>
    ...
    <script>
        function verifyWithGoogle() {
            ...
            fetch('?save_form_data=1', {
                ...
                window.location.href = '<?php echo $rootBase; ?>/auth/google/register.php?role=management';
